...
def getTotalTime(arr: list) -> int:
  ...
  # the idea is to choose the largest two to sum to have the higher penalty
  output = 0
  arr.sort(key=lambda x: -x) # i have to give it a key for ascending order
  # [4, 2, 1, 3] => [1, 2, 3, 4]
  for i in range(1, len(arr)):
      arr[i] += arr[i-1]
      output += arr[i]
 
  return output  #return sum(arr[1:])
  #(An int representing the highest possible total penalty.)
